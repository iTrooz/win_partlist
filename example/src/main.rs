use win_partlist::{list_disks, types::{Disk, PartitionExtra}};
...
fn analyze_disk_data(disks: Vec<Disk>) {
    ...
    for (disk_index, disk) in disks.into_iter().enumerate() {
        println!("Disk {}: {} partition(s)", disk_index, disk.partitions.len());
        ...
        for partition in disk.partitions {
            ...
            match partition.extra {
                PartitionExtra::Gpt(gpt) => {
                    ...
                    println!("    GPT: GUID: {}, Type: {}, Name = {}", gpt.partition_id, gpt.partition_type, name);
                },
                PartitionExtra::Mbr(mbr) => {
                    println!("    MBR: Type: {}, ID: {}", mbr.partition_type, mbr.partition_id);
                },
            }
        }
    }
